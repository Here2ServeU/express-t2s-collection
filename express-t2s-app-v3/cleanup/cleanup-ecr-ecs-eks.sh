...
if [ "$CLUSTER_STATUS" == "ACTIVE" ]; then
  ...
  if [ "$SERVICE_EXISTS" != "MISSING" ] && [ "$SERVICE_EXISTS" != "INACTIVE" ]; then
    echo "Updating and deleting ECS service..."
    ...
    echo "ECS service does not exist or is inactive. Skipping service deletion."
  fi
  ...
  echo "ECS cluster is missing or inactive. Skipping ECS cleanup."
fi
